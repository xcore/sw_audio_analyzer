...
int main(){
  ...
  par {
    ...
    on tile[0]: xscope_handler(c_host_data, i_chan_config, i_error_reporting, 4);

    on tile[1].core[0]: audio_analyzer(i_analysis[0], i_sched0[0], SAMP_FREQ, 0, i_error_reporting[0]);
    on tile[1].core[0]: audio_analyzer(i_analysis[1], i_sched0[1], SAMP_FREQ, 1, i_error_reporting[1]);
    on tile[1].core[0]: analysis_scheduler(i_sched0, 2);

    on tile[1].core[1]: audio_analyzer(i_analysis[2], i_sched1[0], SAMP_FREQ, 2, i_error_reporting[2]);
    on tile[1].core[1]: audio_analyzer(i_analysis[3], i_sched1[1], SAMP_FREQ, 3, i_error_reporting[3]);
